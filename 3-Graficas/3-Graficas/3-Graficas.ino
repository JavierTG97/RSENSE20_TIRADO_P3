...
void setup() {
  ...
  timerAlarmWrite(timer, 100000, true); //Interrupcion = 100.000 conteos (100 ms) e indicamos que cada vez que llegue a ese valor, el conteo se renueve.
...
void loop() {
  ...
  if (interruptCounter > 0) {
    ...
    if (mySensor.accelUpdate() == 0) {
      aX = mySensor.accelX() - offsetX;       
      aY = mySensor.accelY() - offsetY; 
      aZ = mySensor.accelZ() - offsetZ; 
    ...
    Serial.print(aZ);
    Serial.print(";");
    modulo2 = aX*aX + aY*aY + aZ*aZ;
    Serial.print(sqrt(modulo2));
    Serial.println(";");
